...
def get_image_blob(im, target_scale, target_max_size):
    ...
    processed_im, im_scale = prep_im_for_blob(
        # change by jbr, set PIXEL_MEANS=[0,0,0]
        #im, cfg.PIXEL_MEANS, [target_scale], target_max_size
        im, cfg.NORM_MEANS, cfg.NORM_STDS, [target_scale], target_max_size
    )
...
def get_max_shape(im_shapes):
    ...
    # change by jbr, pad the image so they can be divisible by a stride
    if cfg.MSNET.MSNET_ON:
        stride = int(2**(cfg.MSNET.NUM_STAGE+1))
        max_shape[0] = int(np.ceil(max_shape[0] / stride) * stride)
        max_shape[1] = int(np.ceil(max_shape[1] / stride) * stride)
    return max_shape
...
def prep_im_for_blob(im, norm_means, norm_stds, target_sizes, max_size):
    ...
    for c in range(im.shape[-1]):
        im[:,:,c] = (im[:,:,c] - norm_means[c]) / norm_stds[c] 
    im_shape = im.shape
