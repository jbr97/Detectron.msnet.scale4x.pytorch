...
__C.MSNET.MSNET_ON = False
__C.MSNET.NUM_STAGE = 4

# Number of base channels in each stage
# cause each stage has multiple branches
# the real channel number of branch $i is BASE_CHANNEL * (CHANNEL_GROWTH ** $i)
__C.MSNET.BASE_CHANNEL = [256, 32, 32, 32]
__C.MSNET.CHANNEL_GROWTH = 2

# Block Type in the branches in every stage
__C.MSNET.BLOCK_TYPE = 'BasicBlock'

# In each branch, there are BRANCH_DEPTH blocks before fusion
# and there are NUM_BLOCKS such block-and-fusion before Xfusion (with a concat)
__C.MSNET.BRANCH_DEPTH = [4, 4, 4, 4]
__C.MSNET.NUM_BLOCKS = [3, 1, 4, 3]


__C.MSNET.IMAGENET_PRETRAINED_WEIGHTS = ''
...
# change by jbr, set PIXEL_MEANS = 0, add NORM_MEANS & NORM_STDS
__C.PIXEL_MEANS = np.array([[[0, 0, 0]]])
__C.NORM_MEANS = [0.485, 0.456, 0.406]
__C.NORM_STDS = [0.229, 0.224, 0.225]
